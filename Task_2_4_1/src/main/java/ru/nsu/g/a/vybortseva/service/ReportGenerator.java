...
public class ReportGenerator {
     ...
     */
    public void generateHtmlReport(Map<Integer, Map<String, Map<String, StudentTaskData>>> allData, Config config) throws IOException {
        ...
        html.append("        th, td { border: 1px solid #999; padding: 8px 10px; text-align: center; }\n");
        ...
        html.append("    <p class='note'>Дата формирования отчета: ").append(today.format(DATE_FORMATTER)).append("</p>\n");
    ...
    private void generateTaskTable(StringBuilder html, String taskId,
        ...
        html.append("<h3>Лабораторная ").append(taskId).append(" (").append(taskTitle).append(")</h3>\n");
        html.append("<p class='note'>Мягкий дедлайн: ").append(softDate).append(" | Жесткий дедлайн: ").append(hardDate).append("</p>\n");
        ...
        for (var studentEntry : groupData.entrySet()) {
            ...
            if (data != null && data.grade != null) {
                ...
                html.append("            <td class='date'>").append(commitDateStr).append("</td>\n");
                html.append("            <td class='").append(data.buildSuccess ? "ok" : "fail").append("'>").append(data.buildSuccess ? "+" : "-").append("</td>\n");
                html.append("            <td class='").append(data.docSuccess ? "ok" : "fail").append("'>").append(data.docSuccess ? "+" : "-").append("</td>\n");
                html.append("            <td class='").append(data.styleSuccess ? "ok" : "fail").append("'>").append(data.styleSuccess ? "+" : "-").append("</td>\n");
                html.append("            <td>").append(testStr).append("</td>\n");
                html.append("            <td>").append(String.format("%.1f", data.grade.basePoints)).append("</td>\n");
                html.append("            <td>").append(String.format("%.1f", data.grade.softPoints)).append("</td>\n");
                html.append("            <td>").append(String.format("%.1f", data.grade.hardPoints)).append("</td>\n");
                html.append("            <td>").append(String.format("%.1f", data.grade.bonusPoints)).append("</td>\n");
                html.append("            <td><b>").append(String.format("%.1f", data.grade.totalPoints)).append("</b></td>\n");
                html.append("        </tr>\n");
            }
        }
        html.append("    </tbody>\n");
    ...
    private void generateSummaryTable(StringBuilder html, int groupNumber,
        ...
        html.append("<p class='note'>Дата формирования: ").append(today.format(DATE_FORMATTER)).append("</p>\n");
        ...
        html.append("            <th colspan='").append(taskIds.size()).append("'>Баллы за задачи</th>\n");
        ...
        for (var studentEntry : groupData.entrySet()) {
            ...
            for (String taskId : taskIds) {
                ...
                total += score;
                html.append("            <td>").append(String.format("%.1f", score)).append("</td>\n");
            }

            for (CheckPoint point : config.getPoints()) {
                double earnedUpToPoint = calculateEarnedUpToPoint(tasks, taskIds, config, point);
                html.append("            <td>").append(String.format("%.1f", earnedUpToPoint)).append("</td>\n");
            }
